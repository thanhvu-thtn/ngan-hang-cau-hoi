<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('questions.store') }}" method="POST" id="main-form">
                @csrf

                {{-- ── DỮ LIỆU ĐIỀU HƯỚNG ẨN ── --}}
                <input type="hidden" name="nav_source"           value="{{ $navigation['source'] }}">
                <input type="hidden" name="nav_search_code"      value="{{ $navigation['search_code'] }}">
                <input type="hidden" name="nav_shared_context_id" value="{{ $navigation['shared_context_id'] }}">
                @foreach ($navigation['objective_ids'] as $id)
                    <input type="hidden" name="nav_objective_ids[]" value="{{ $id }}">
                @endforeach

                {{-- ── SHARED CONTEXT ID (data field, lưu vào DB) ── --}}
                @if ($navigation['shared_context_id'])
                    <input type="hidden" name="shared_context_id" value="{{ $navigation['shared_context_id'] }}">
                @endif

                {{-- ── TRẠNG THÁI & LOẠI CÂU HỎI (ẩn) ── --}}
                {{-- Trạng thái mặc định PENDING, không cho người dùng thay đổi khi tạo mới --}}
                <input type="hidden" name="question_status_id"  value="{{ $pendingStatus?->id }}">
                {{-- question_type_id được JS đồng bộ từ select --}}
                <input type="hidden" name="question_type_id"    id="hidden_question_type_id" value="{{ old('question_type_id') }}">
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                        <h3 class="text-lg font-bold text-blue-800 mb-4 flex items-center">
                            <span class="bg-blue-100 text-blue-800 rounded-full w-7 h-7 flex items-center justify-center mr-2">1</span>
                            Thông tin cơ bản
                        </h3>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700">
                                    Mã câu hỏi <span class="text-red-500">*</span>
                                </label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700">
                                    Kiểu câu hỏi <span class="text-red-500">*</span>
                                </label>
                                <select name="question_type_code" id="question_type_code" required
                                    ...
                                    @foreach ($types as $type)
                                        <option value="{{ $type->code }}"
                                            data-id="{{ $type->id }}"
                                            {{ old('question_type_code') == $type->code ? 'selected' : '' }}>
                                            {{ $type->name }} ({{ $type->code }})
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                        <h3 class="text-lg font-bold text-blue-800 mb-4 flex items-center">
                            <span class="bg-blue-100 text-blue-800 rounded-full w-7 h-7 flex items-center justify-center mr-2">2</span>
                            Phần dẫn (Stem)
                        </h3>
                        <div class="mb-2 flex space-x-2">
                            ...
                            <button type="button" onclick="showPreview('stem')"
                                class="text-xs px-2 py-1 bg-green-100 text-green-700 rounded font-bold">Xem trước (Preview)</button>
                        </div>
                        <textarea id="stem" name="stem" rows="6"
                            class="w-full border-gray-300 rounded-md shadow-sm">{{ old('stem') }}</textarea>
                        @error('stem')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div id="choices-container" class="space-y-6">
                        ...
                        <div id="block-MC"
                                ...
                                @foreach (range(0, 3) as $index)
                                    ...
                                    <div class="mc-choice-item p-4 border rounded-lg bg-gray-50 relative"
                                        ...
                                        <div class="flex justify-between items-center mb-2">
                                            ...
                                            <div class="flex items-center">
                                                <input type="hidden"
                                                    ...
                                                    value="{{ old("choices.$index.is_true", 0) }}">
                                                <input type="radio" name="mc_correct_answer"
                                                    value="{{ $index }}"
                                                    @checked(old("choices.$index.is_true") == 1)
                                                    onchange="
                                        ...
                                        <div class="mb-1 flex space-x-2">
                                            <button type="button" onclick="initTinyMCE('{{ $editorId }}')"
                                                class="text-xs px-2 py-1 bg-gray-200 rounded">Bật soạn thảo</button>
                                            <button type="button" onclick="tinymce.execCommand('mceRemoveEditor', false, '{{ $editorId }}')"
                                                class="text-xs px-2 py-1 bg-red-50 text-red-600 rounded">Tắt</button>
                                            <button type="button" onclick="showPreview('{{ $editorId }}')"
                                                class="text-xs px-2 py-1 bg-green-100 text-green-700 rounded font-bold">Xem trước</button>
                                        </div>

                                        {{-- Textarea nội dung đáp án --}}
                                        <textarea id="{{ $editorId }}" name="choices[{{ $index }}][content]"
                                            class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                            rows="3">{{ old("choices.$index.content") }}</textarea>
                                    </div>
                                @endforeach
                            </div>
                            ...
                            <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6 border-t pt-4">
                                    ...
                                    <select name="question_layout_id"
                                        ...
                                        @foreach ($layouts as $layout)
                                            <option value="{{ $layout->id }}"
                                                {{ old('question_layout_id') == $layout->id ? 'selected' : '' }}>
                                ...
                                <div>
                                    ...
                                    <input type="number" name="layout_ratio" value="{{ old('layout_ratio') }}"
                        ...
                        {{-- ---- BLOCK SA (Short Answer) ---- --}}
                        <div id="block-SA"
                            class="type-block hidden bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                            <h4 class="font-bold text-gray-700 mb-2 text-left">Đáp án ngắn (Số)</h4>
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                        <h3 class="text-lg font-bold text-blue-800 mb-4 flex items-center">
                            <span class="bg-blue-100 text-blue-800 rounded-full w-7 h-7 flex items-center justify-center mr-2">3</span>
                        ...
                        <textarea id="explanation" name="explanation" rows="4"
                            class="w-full border-gray-300 rounded-md shadow-sm">{{ old('explanation') }}</textarea>
                    </div>
        ...
        <div
            ...
            <div class="p-6 overflow-y-auto flex-grow bg-gray-50">
                <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200 min-h-full">
                    <x-treeview.objective-selector :items="$treeData" id_prefix="create_" />
                </div>
            </div>
...
<script>
    // ── Dữ liệu PHP → JS ──────────────────────────────────────────
    const currentTypeCode = @json(old('question_type_code'));
    ...
    // ── Cập nhật hidden question_type_id từ data-id trên option ──
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // 1. Hiển thị đúng block nếu có old value (sau validation fail)
        if (currentTypeCode) {
            toggleQuestionBlocks(currentTypeCode);
            updateQuestionTypeId(currentTypeCode);
        }

        // 2. Lắng nghe thay đổi loại câu hỏi (đã inline trong onchange, nhưng giữ đây để nhất quán)
