...
class ObjectiveController extends Controller
{
    ...
    public function create()
    {
        ...
        $topicTypes = TopicType::all();
        $topics = Topic::all();
        $topicContents = TopicContent::all();

        return view('objectives.create', compact('grades', 'topicTypes', 'topics', 'topicContents'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'topic_content_id.exists' => 'Nội dung chuyên đề được chọn không tồn tại trong hệ thống.',
            'code.required' => 'Vui lòng nhập mã định danh cho yêu cầu cần đạt.',
            'code.unique' => 'Mã định danh này đã tồn tại, vui lòng chọn mã khác.',
        ...
        try {
            ...
            return redirect()->route('objectives.index')
                ->with('success', 'Đã thêm Yêu cầu cần đạt thành công!');

        } catch (\Exception $e) {
            // Nếu có lỗi hệ thống, quay lại trang trước, giữ nguyên dữ liệu đã nhập (withInput) và báo lỗi
